...
1. The main problem with the code is when it is compiled using MSVC++ 5.0.  If you're compiling under Unix, it shouldn't be a problem running it. 

2. The window[] array initialization is slightly wrong: e.g.
window[i] = 0.5 - 0.5 * cos (2.0 * PI * (double) i / nplot)
should be
window[i] = 0.5 - 0.5 * cos (2.0 * PI * (double) (i + 1) / (nplot + 1))
and there are no Blackman or Blackman-Harris windows defined.

3. The function "analytic" is far slower than it should be.  Let me know and I'll show you how to speed it up.
