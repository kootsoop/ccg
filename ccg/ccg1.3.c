...
#define  NMAX           32768	/* Maximum signal length */
...
**************************************************************/
/* modified 28/3/96 SJS:- shortened loops to speed it up
   #define NEWANAL to use this modified version.  */
analytic (xr, xi, nsam)
    double  xr[],
            xi[];		/* real and imag. arrays */
int     nsam;			/* length of arrays */

#define	 MAXHILBLENGTH 	201	/* Hilbert transformer length */
#define NEWANAL 
{
    int i, j, j1, jmax;
    ...
    for (i = 0; i < nsam; i++) {
	xi[i] = 0.0;
	j1 = i-MAXHILBLENGTH+1;
	j1 += (!((j-i)&1));
	jmax = i+MAXHILBLENGTH-1;
	for (j = j1; j < ((jmax<nsam)?jmax:nsam); j+=2) {
